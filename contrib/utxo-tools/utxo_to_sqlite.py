...
The input UTXO set can be generated by Tessera with the `dumptxoutset` RPC:
$ tessera-cli dumptxoutset ~/utxos.dat latest
...
UTXO_DUMP_VERSION = 2
NET_MAGIC_BYTES = {
    b"\xf1\xc0\x0c\x19": "Mainnet",
    b"\x0f\xc0\x0c\x19": "Testnet",
    b"\xfa\xc0\x0c\x19": "Regtest",
}
...
def decompress_script(f):
    ...
    size = read_varint(f)  # sizes 0-1 encode compressed script types
    if size == 0:  # P2PKH
        return bytes([0x76, 0xa9, 20]) + f.read(20) + bytes([0x88, 0xac])
    elif size == 1:  # P2SH
        return bytes([0xa9, 20]) + f.read(20) + bytes([0x87])
    else:  # others (P2WPKH, bare multisig etc.)
        size -= 6
