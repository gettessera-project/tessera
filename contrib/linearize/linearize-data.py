...
def calc_hash_str(blk_hdr):
    # Tessera block hashes are a single SHA3-256 over the serialized header
    # (CBlockHeader::GetHash via HashWriter), not Bitcoin's double SHA-256.
    blk_hdr_hash = hashlib.sha3_256(blk_hdr).digest()
...
class BlockDataCopier:
    ...
    def run(self):
        while self.blkCountOut < len(self.blkindex):
            ...
            inLen = su[0] - 80 # length without header (Tessera uses Bitcoin's 80-byte header)
...
if __name__ == '__main__':
    ...
    if 'netmagic' not in settings:
        settings['netmagic'] = 'f1c00c19'
    if 'genesis' not in settings:
        settings['genesis'] = 'facbbceb849f5a86c2e0e3fc3a4a2f13d738c613b7351e73a30449788d8a6ab3'
