# CJDNS support in Tessera

It is possible to run Tessera over CJDNS, an encrypted IPv6 network that
...
done outside of Tessera, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Tessera network and individual nodes.
...
## Run Tessera with CJDNS

Once you are connected to the CJDNS network, the following Tessera
...
When enabled, this option tells Tessera that it is running in an
...
IPv6 local network. This helps Tessera perform better address management:
...
CJDNS support was added to Tessera in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `tessera-cli -addrinfo` to see the
...
## CJDNS-related information in Tessera

There are several ways to see your CJDNS address in Tessera:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `tessera-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `tessera-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `tessera-cli help getnodeaddresses` for details.
