...
The headless daemon `tesserad` has the JSON-RPC API enabled by default, the GUI
`tessera-qt` has it disabled by default. This can be changed with the `-server`
...
This is the endpoint used by tessera-cli when a `-rpcwallet=` parameter is passed in.
...
```sh
# Get block count from the / endpoint when rpcuser=alice and rpcport=9332
$ curl --user alice --data-binary '{"jsonrpc": "2.0", "id": "0", "method": "getblockcount", "params": []}' -H 'content-type: application/json' localhost:9332/

# Get balance from the /wallet/walletname endpoint when rpcuser=alice, rpcport=9332 and rpcwallet=mywallet
$ curl --user alice --data-binary '{"jsonrpc": "2.0", "id": "0", "method": "getbalance", "params": []}' -H 'content-type: application/json' localhost:9332/wallet/mywallet
...
```sh
# "params": ["mywallet", false, false, "", false, true]
tessera-cli createwallet mywallet false false "" false true

# "params": {"wallet_name": "mywallet", "load_on_startup": true}
tessera-cli -named createwallet wallet_name=mywallet load_on_startup=true

# "params": {"args": ["mywallet"], "load_on_startup": true}
tessera-cli -named createwallet mywallet load_on_startup=true
```


## Versioning

The RPC interface might change from one major version of Tessera to the
...
The RPC interface allows other programs to control Tessera,
including the ability to spend funds from your wallets, affect consensus
verification, read private data, and otherwise perform operations that
can cause loss of money, data, or privacy.  This section suggests how
you should use and configure Tessera to reduce the risk that its
RPC interface will be abused.

- **Securing the executable:** Anyone with physical or remote access to
  the computer, container, or virtual machine running Tessera can
  compromise either the whole program or just the RPC interface.  This
  includes being able to record any passphrases you enter for unlocking
  your encrypted wallets or changing settings so that your Tessera
  program tells you that certain transactions have multiple
  confirmations even when they aren't part of the best block chain.  For
  this reason, you should not use Tessera for security sensitive
...
- **Securing local network access:** By default, the RPC interface can
  ...
  an RPC interface on the same port as used by Tessera in order to
  trick you into revealing your authentication credentials.  For this
  reason, it is important to only use Tessera for
  security-sensitive operations on a computer whose other programs you
  trust.

- **RPC Credentials Security Boundary:** Any client with valid RPC credentials
  should be treated as having significant control over both the Tessera node
  and the filesystem resources accessible by the `tesserad` process. RPC commands
  can load wallet files from paths that the `tesserad` process has permission to
  ...
  not only the Tessera node, but also the machine it is running on. Tessera
...
- **Securing remote network access:** You may optionally allow other
  computers to remotely control Tessera by setting the `rpcallowip`
  ...
  Internet.**  Although Tessera's RPC interface does use
  ...
  `tesserad -help` for more information about these settings and other
  settings described in this document.

    Related, if you use Tessera inside a Docker container, you may
    need to expose the RPC port to the host system.  The default way to
    do this in Docker also exposes the port to the public Internet.
    Instead, expose it only on the host system's localhost, for example:
    `-p 127.0.0.1:8332:8332`

- **Secure authentication:** By default, when no `rpcpassword` is specified, Tessera generates unique
  login credentials each time it restarts and puts them into a file
  readable only by the user that started Tessera, allowing any of
  that user's RPC clients with read access to the file to login
  automatically.  The file is `.cookie` in the Tessera
  configuration directory, and using these credentials is the preferred
  RPC authentication method.  If you need to generate static login
  credentials for your programs, you can use the script in the
  `share/rpcauth` directory in the Tessera source tree.  As a final
