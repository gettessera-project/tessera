...
Tessera uses [Doxygen](https://www.doxygen.nl/) to generate its official documentation.
...
configuration options and toggled while tesserad is running with the `logging`
RPC.  For instance, launching tesserad with `-debug` or `-debug=1` will turn on
...
with "play coins" on a test network.
...
Tessera is a multi-threaded application, and deadlocks or other
...
You can then use the `-debug=lock` configuration option at tesserad startup or
`tessera-cli logging '["lock"]'` at runtime to turn on lock contention logging.
It can be toggled off again with `tessera-cli logging [] '["lock"]'`.
...
# A coverage report will now be accessible at `./build/test_tessera.coverage/index.html`,
...
```shell
llvm-cov show \
    --object=build/bin/test_tessera \
    --object=build/bin/tesserad \
    -Xdemangler=llvm-cxxfilt \
    --instr-profile=build/coverage.profdata \
    --ignore-filename-regex="src/crc32c/|src/leveldb/|src/minisketch/|src/ML-DSA-44/|src/ML-KEM-768/|src/test/" \
    --format=html \
    --show-instantiation-summary \
    --show-line-counts-or-regions \
    --show-expansions \
    --output-dir=build/coverage_report \
    --project-title="Tessera Coverage Report"
```
...
llvm-cov show \
    ...
    --show-expansions \
    --output-dir=build/coverage_report \
    --project-title="Tessera Fuzz Coverage Report"
```
...
To profile a running tesserad process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep tesserad` -- sleep 60
```
...
```shell
$ valgrind --suppressions=test/sanitizer_suppressions/valgrind.supp build/bin/test_tessera
$ valgrind --suppressions=test/sanitizer_suppressions/valgrind.supp --leak-check=full \
      --show-leak-kinds=all build/bin/test_tessera --log_level=test_suite
$ valgrind -v --leak-check=full build/bin/tesserad -printtoconsole
$ ./build/test/functional/test_runner.py --valgrind
```

### Sanitizers

Tessera can be compiled with various "sanitizers" enabled, which add
...
pay attention to for reviewers of Tessera code.
...
- [Main thread (`tesserad`)](https://doxygen.bitcoincore.org/tesserad_8cpp.html#main)
  : Started from `main()` in `tesserad.cpp`. Responsible for starting up and
...
- For `strprintf`, `LogInfo`, `LogDebug`, etc formatting characters don't need size specifiers (hh, h, l, ll, j, z, t, L) for arithmetic types.
  ...
  - *Rationale*: Tessera uses tinyformat, which is type safe. Leave them out to avoid confusion.
...
Normally, these are maintained by active developers of Tessera, in which case
...
a Tessera subtree, so that they can be integrated quickly. Cosmetic changes
...
The tool instructions also include a list of the subtrees managed by Tessera.
...
- src/leveldb
  - Upstream at https://github.com/google/leveldb ; maintained by Google. Open
    important PRs to the Tessera subtree to avoid delay.
...
file descriptors, it will cause problems with Tessera's `select()` loop, because
...
would be to revert the upstream fix before applying the updates to Tessera's
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `tessera-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `tessera-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Tessera, the wallet was always
...
Tessera uses a structured process for deprecating and removing features to give
...
- The RPC help text must mention the deprecation and the `-deprecatedrpc=<feature>` flag
  ...
  ```
  "\nDeprecated in v25.0, use the `newfoo` RPC instead. Start tesserad with"
