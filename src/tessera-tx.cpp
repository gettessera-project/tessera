...
static void SetupTesseraTxArgs(ArgsManager &argsman)
{
...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.GetBoolArg("-version", false)) {
        ...
        std::string strUsage = CLIENT_NAME " tessera-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.GetBoolArg("-version", false)) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "The tessera-tx tool is used for creating and modifying tessera transactions.\n\n"
                "tessera-tx can be used with \"<hex-tx> [commands]\" to update a hex-encoded tessera transaction, or with \"-create [commands]\" to create a hex-encoded tessera transaction.\n"
                "\n"
                "Usage: tessera-tx [options] <hex-tx> [commands]\n"
                "or:    tessera-tx [options] -create [commands]\n"
                "\n";
...
static void MutateTxAddOutAddr(CMutableTransaction& tx, const std::string& strInput)
{
    ...
    CTxDestination destination = DecodeDestination(strAddr, Params());
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded tessera transaction
