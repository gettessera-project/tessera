...
//
// SHA3-512 (FIPS 202, Keccak-f[1600]) for Tessera's RNG. Self-contained: this
// file carries its own copy of the Keccak-f permutation (internal linkage), so
// the RNG's hashing does not depend on the consensus SHA3-256 (crypto/sha3.cpp).
// Verified bit-exact against Python hashlib.sha3_512 (test/test_sha3_512.cpp).
...
namespace {
//! The Keccak-f[1600] permutation, in place over the 25-lane state. Internal to
//! this file (a duplicate of crypto/sha3.cpp's, to keep the RNG hash decoupled
//! from the consensus hash).
void KeccakF512(uint64_t (&st)[25])
{
...
SHA3_512& SHA3_512::Write(const unsigned char* dataptr, size_t len)
{
    m_bytes += len;
    std::span<const unsigned char> data{dataptr, len};
...
void SHA3_512::Finalize(unsigned char hash[OUTPUT_SIZE])
{
