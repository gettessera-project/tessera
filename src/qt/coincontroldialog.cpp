...
{
    GUIUtil::setClipboard(TesseraUnits::removeSpaces(contextMenuItem->text(COLUMN_AMOUNT)));
}
...
void CoinControlDialog::updateLabels(CCoinControl& m_coin_control, WalletModel *model, QDialog* dialog)
{
    ...
    TesseraUnit nDisplayUnit = TesseraUnit::TSR;
    ...
    l2->setText(TesseraUnits::formatWithUnit(nDisplayUnit, nAmount));        // Amount
    l3->setText(TesseraUnits::formatWithUnit(nDisplayUnit, nPayFee));        // Fee
    l4->setText(TesseraUnits::formatWithUnit(nDisplayUnit, nAfterFee));      // After Fee
    l5->setText(((nBytes > 0) ? ASYMP_UTF8 : "") + QString::number(nBytes));        // Bytes
    l8->setText(TesseraUnits::formatWithUnit(nDisplayUnit, nChange));        // Change
    if (nPayFee > 0)
    {
...
void CoinControlDialog::updateView()
{
    ...
    TesseraUnit nDisplayUnit = model->getOptionsModel()->getDisplayUnit();

    for (const auto& coins : model->wallet().listCoins()) {
        CCoinControlWidgetItem* itemWalletAddress{nullptr};
        QString sWalletAddress = QString::fromStdString(EncodeDestination(coins.first, Params()));
        ...
        for (const auto& outpair : coins.second) {
            ...
            {
                sAddress = QString::fromStdString(EncodeDestination(outputAddress, Params()));
            ...
            // amount
            itemOutput->setText(COLUMN_AMOUNT, TesseraUnits::format(nDisplayUnit, out.txout.nValue));
        ...
        if (treeMode)
        {
            ...
            itemWalletAddress->setText(COLUMN_AMOUNT, TesseraUnits::format(nDisplayUnit, nSum));
