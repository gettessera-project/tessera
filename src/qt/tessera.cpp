...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaType<TesseraUnit>("TesseraUnit");
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. tessera_de.qm (shortcut "de" needs to be defined in tessera.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. tessera_de_DE.qm (shortcut "de_DE" needs to be defined in tessera.qrc)
...
static const char* qt_argv = "tessera-qt";

TesseraApplication::TesseraApplication()
...
void TesseraApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the TesseraApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", TesseraGUI::DEFAULT_UIPLATFORM);
...
bool TesseraApplication::createOptionsModel(bool resetSettings)
{
...
void TesseraApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new TesseraGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &TesseraGUI::quitRequested, this, &TesseraApplication::requestShutdown);
...
void TesseraApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void TesseraApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool TesseraApplication::baseInitialize()
{
    return node().baseInitialize();
}

void TesseraApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &TesseraApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &TesseraApplication::handleRunawayException);
    connect(this, &TesseraApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &TesseraApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void TesseraApplication::parameterSetup()
{
...
void TesseraApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void TesseraApplication::requestInitialize()
{
...
void TesseraApplication::requestShutdown()
{
...
void TesseraApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
...
void TesseraApplication::handleRunawayException(const QString &message)
{
...
void TesseraApplication::handleNonFatalException(const QString& message)
{
...
WId TesseraApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool TesseraApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", TesseraGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(tessera);
    Q_INIT_RESOURCE(tessera_locale);
    ...
    // Tessera has no payment server, so there are no BIP-21 URI tokens to allow.
    for (int i = 1; i < argc; i++) {
        QString arg(argv[i]);
        if (!arg.startsWith("-")) {
            InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see tessera-qt -h for a list of options.", argv[i])));
            QMessageBox::critical(nullptr, CLIENT_NAME,
                                  // message cannot be translated because translations have not been initialized
                                  QString::fromStdString("Command line contains unexpected token '%1', see tessera-qt -h for a list of options.").arg(QString::fromStdString(argv[i])));
            return EXIT_FAILURE;
        }
    }
    ...
    /// 6-7. Parse tessera.conf, determine network, switch to network specific
