    ...
    if (all_from_me || !any_from_me) {
        ...
        for(unsigned int i = 0; i < wtx.tx->vout.size(); i++)
        {
            ...
            if (all_from_me) {
                ...
                // Otherwise, someone just sent tesseras to a change address, which should be shown
                ...
                {
                    // Sent to Tessera Address
                    sub.type = TransactionRecord::SendToAddress;
                    sub.address = EncodeDestination(wtx.txout_address[i], Params());
                }
                else
                {
