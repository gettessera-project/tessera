    ...
    if(role == Qt::DisplayRole || role == Qt::EditRole)
    {
        ...
        case Amount:
            if (rec->recipient.amount == 0 && role == Qt::DisplayRole)
                ...
                return TesseraUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount, false, TesseraUnits::SeparatorStyle::NEVER);
            else
                return TesseraUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount);
        }
    }
...
QString RecentRequestsTableModel::getAmountTitle()
{
    ...
    return tr("Requested") +
           QLatin1String(" (") +
           TesseraUnits::shortName(this->walletModel->getOptionsModel()->getDisplayUnit()) +
           QLatin1Char(')');
}
...
bool RecentRequestsTableModel::removeRows(int row, int count, const QModelIndex &parent)
{
    ...
    if(count > 0 && row >= 0 && (row+count) <= list.size())
    {
        for (int i = 0; i < count; ++i)
        {
            ...
            if (!walletModel->wallet().setAddressReceiveRequest(DecodeDestination(rec->recipient.address.toStdString(), Params()), ToString(rec->id), ""))
                return false;
        }
...
void RecentRequestsTableModel::addNewRequest(const SendCoinsRecipient &recipient)
{
    ...
    if (!walletModel->wallet().setAddressReceiveRequest(DecodeDestination(recipient.address.toStdString(), Params()), ToString(newEntry.id), ss.str()))
        return;
