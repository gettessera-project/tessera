...
void SignVerifyMessageDialog::on_signMessageButton_SM_clicked()
{
    ...
    CTxDestination destination = DecodeDestination(ui->addressIn_SM->text().toStdString(), Params());
    if (!IsValidDestination(destination)) {
        ui->statusLabel_SM->setStyleSheet("QLabel { color: red; }");
        ui->statusLabel_SM->setText(tr("The entered address is invalid.") + QString(" ") + tr("Please check the address and try again."));
        return;
    }
    // Tessera addresses are witness-v0 key hashes (P2WPKH); map either a witness
    // key hash or a legacy pubkey hash to the underlying CKeyID.
    std::optional<CKeyID> keyid;
    if (const auto* wpkh = std::get_if<WitnessV0KeyHash>(&destination)) {
        keyid = ToKeyID(*wpkh);
    } else if (const auto* pkh = std::get_if<PKHash>(&destination)) {
        keyid = ToKeyID(*pkh);
    }
    if (!keyid) {
        ui->addressIn_SM->setValid(false);
        ui->statusLabel_SM->setStyleSheet("QLabel { color: red; }");
        ui->statusLabel_SM->setText(tr("The entered address does not refer to a key. Please check the address and try again."));
        return;
    }
    ...
    SigningResult res = model->wallet().signMessage(message, PKHash(*keyid), signature);
