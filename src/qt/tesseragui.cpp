 ...
 *
 * Ref: https://github.com/tessera/tessera/pull/1026
 */
static constexpr int64_t MAX_BLOCK_TIME_GAP = 90 * 60;

const std::string TesseraGUI::DEFAULT_UIPLATFORM =
...
TesseraGUI::TesseraGUI(interfaces::Node& node, const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    m_node(node),
    ...
    m_network_style(networkStyle)
{
    ...
    if(enableWallet)
    {
        ...
        connect(walletFrame, &WalletFrame::createWalletButtonClicked, this, &TesseraGUI::createWallet);
    ...
    connect(labelBlocksIcon, &GUIUtil::ClickableLabel::clicked, this, &TesseraGUI::showModalOverlay);
    connect(progressBar, &GUIUtil::ClickableProgressBar::clicked, this, &TesseraGUI::showModalOverlay);
...
void TesseraGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Tessera address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and tessera: URIs)"));
    ...
    connect(overviewAction, &QAction::triggered, this, &TesseraGUI::gotoOverviewPage);
    connect(sendCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(sendCoinsAction, &QAction::triggered, [this]{ gotoSendCoinsPage(); });
    connect(receiveCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(receiveCoinsAction, &QAction::triggered, this, &TesseraGUI::gotoReceiveCoinsPage);
    connect(historyAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(historyAction, &QAction::triggered, this, &TesseraGUI::gotoHistoryPage);
#endif // ENABLE_WALLET
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Tessera addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message…"), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Tessera addresses"));
    ...
    openAction->setStatusTip(tr("Open a tessera: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Tessera command-line options").arg(CLIENT_NAME));
    ...
    connect(quitAction, &QAction::triggered, this, &TesseraGUI::quitRequested);
    connect(aboutAction, &QAction::triggered, this, &TesseraGUI::aboutClicked);
    connect(aboutQtAction, &QAction::triggered, qApp, QApplication::aboutQt);
    connect(optionsAction, &QAction::triggered, this, &TesseraGUI::optionsClicked);
    connect(showHelpMessageAction, &QAction::triggered, this, &TesseraGUI::showHelpMessageClicked);
    connect(openRPCConsoleAction, &QAction::triggered, this, &TesseraGUI::showDebugWindow);
    ...
    if(walletFrame)
    {
        ...
        connect(openAction, &QAction::triggered, this, &TesseraGUI::openClicked);
        connect(m_open_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const auto& [path, info] : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, path] {
                    ...
                    connect(activity, &OpenWalletActivity::opened, this, &TesseraGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_create_wallet_action, &QAction::triggered, this, &TesseraGUI::createWallet);
        connect(m_restore_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &RestoreWalletActivity::restored, this, &TesseraGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_mask_values_action, &QAction::toggled, this, &TesseraGUI::setPrivacy);
        connect(m_mask_values_action, &QAction::toggled, this, &TesseraGUI::enableHistoryAction);
    }
#endif // ENABLE_WALLET

    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_C), this), &QShortcut::activated, this, &TesseraGUI::showDebugWindowActivateConsole);
    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_D), this), &QShortcut::activated, this, &TesseraGUI::showDebugWindow);
}

void TesseraGUI::createMenuBar()
{
    appMenuBar = menuBar();
...
void TesseraGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        connect(m_wallet_selector, qOverload<int>(&QComboBox::currentIndexChanged), this, &TesseraGUI::setCurrentWalletBySelectorIndex);
...
void TesseraGUI::setClientModel(ClientModel *_clientModel, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if(_clientModel)
    {
        ...
        connect(_clientModel, &ClientModel::numConnectionsChanged, this, &TesseraGUI::setNumConnections);
        connect(_clientModel, &ClientModel::networkActiveChanged, this, &TesseraGUI::setNetworkActive);
        ...
        connect(_clientModel, &ClientModel::numBlocksChanged, this, &TesseraGUI::setNumBlocks);
        ...
        connect(_clientModel, &ClientModel::showProgress, this, &TesseraGUI::showProgress);
...
#ifdef ENABLE_WALLET
void TesseraGUI::enableHistoryAction(bool privacy)
{
...
void TesseraGUI::setWalletController(WalletController* wallet_controller, bool show_loading_minimized)
{
    ...
    GUIUtil::ExceptionSafeConnect(wallet_controller, &WalletController::walletAdded, this, &TesseraGUI::addWallet);
    connect(wallet_controller, &WalletController::walletRemoved, this, &TesseraGUI::removeWallet);
...
WalletController* TesseraGUI::getWalletController()
{
    return m_wallet_controller;
}

void TesseraGUI::addWallet(WalletModel* walletModel)
{
    ...
    connect(wallet_view, &WalletView::outOfSyncWarningClicked, this, &TesseraGUI::showModalOverlay);
    connect(wallet_view, &WalletView::transactionClicked, this, &TesseraGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::coinsSent, this, &TesseraGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::message, [this](const QString& title, const QString& message, unsigned int style) {
        this->message(title, message, style);
    });
    connect(wallet_view, &WalletView::encryptionStatusChanged, this, &TesseraGUI::updateWalletStatus);
    connect(wallet_view, &WalletView::incomingTransaction, this, &TesseraGUI::incomingTransaction);
    connect(this, &TesseraGUI::setPrivacy, wallet_view, &WalletView::setPrivacy);
    wallet_view->setPrivacy(isPrivacyModeActivated());
    const QString display_name = walletModel->getDisplayName();
    m_wallet_selector->addItem(display_name, QVariant::fromValue(walletModel));
}

void TesseraGUI::removeWallet(WalletModel* walletModel)
{
...
void TesseraGUI::setCurrentWallet(WalletModel* wallet_model)
{
    if (!walletFrame || !m_wallet_controller) return;
...
void TesseraGUI::setCurrentWalletBySelectorIndex(int index)
{
    WalletModel* wallet_model = m_wallet_selector->itemData(index).value<WalletModel*>();
    if (wallet_model) setCurrentWallet(wallet_model);
}

void TesseraGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void TesseraGUI::setWalletActionsEnabled(bool enabled)
{
...
void TesseraGUI::createTrayIcon()
{
...
void TesseraGUI::createTrayIconMenu()
{
#ifndef Q_OS_MACOS
    if (!trayIcon) return;
#endif // Q_OS_MACOS
    ...
    show_hide_action = trayIconMenu->addAction(QString(), this, &TesseraGUI::toggleHidden);
...
void TesseraGUI::optionsClicked()
{
    openOptionsDialogWithTab(OptionsDialog::TAB_MAIN);
}

void TesseraGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void TesseraGUI::showDebugWindow()
{
    GUIUtil::bringToFront(rpcConsole);
    Q_EMIT consoleShown(rpcConsole);
}

void TesseraGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TabTypes::CONSOLE);
    showDebugWindow();
}

void TesseraGUI::showHelpMessageClicked()
{
    GUIUtil::bringToFront(helpMessageDialog);
}

#ifdef ENABLE_WALLET
void TesseraGUI::openClicked()
{
...
void TesseraGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void TesseraGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void TesseraGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void TesseraGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void TesseraGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void TesseraGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
#endif // ENABLE_WALLET

void TesseraGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to Tessera network.", "", count);
    } else {
        //: A substring of the tooltip.
...
void TesseraGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void TesseraGUI::setNetworkActive(bool network_active)
{
    updateNetworkState();
...
void TesseraGUI::updateHeadersSyncProgressLabel()
{
...
void TesseraGUI::updateHeadersPresyncProgressLabel(int64_t height, const QDateTime& blockDate)
{
...
void TesseraGUI::openOptionsDialogWithTab(OptionsDialog::Tab tab)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;

    auto dlg = new OptionsDialog(this, enableWallet);
    connect(dlg, &OptionsDialog::quitOnReset, this, &TesseraGUI::quitRequested);
...
void TesseraGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, SyncType synctype, SynchronizationState sync_state)
{
...
void TesseraGUI::createWallet()
{
#ifdef ENABLE_WALLET
    auto activity = new CreateWalletActivity(getWalletController(), this);
    connect(activity, &CreateWalletActivity::created, this, &TesseraGUI::setCurrentWallet);
    connect(activity, &CreateWalletActivity::created, rpcConsole, &RPCConsole::setCurrentWallet);
    activity->create();
#endif // ENABLE_WALLET
}

void TesseraGUI::message(const QString& title, QString message, unsigned int style, bool* ret, const QString& detailed_message)
{
...
void TesseraGUI::changeEvent(QEvent *e)
{
    ...
    if(e->type() == QEvent::WindowStateChange)
    {
        if(clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray())
        {
            ...
            {
                QTimer::singleShot(0, this, &TesseraGUI::hide);
                e->ignore();
            }
            else if((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized())
            {
                QTimer::singleShot(0, this, &TesseraGUI::show);
                e->ignore();
            }
        }
    }
...
void TesseraGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MACOS // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void TesseraGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void TesseraGUI::incomingTransaction(const QString& date, TesseraUnit unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& walletName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(TesseraUnits::formatWithUnit(unit, amount, true));
...
void TesseraGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void TesseraGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool TesseraGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool TesseraGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void TesseraGUI::setHDStatus(bool privkeyDisabled, int hdEnabled)
{
...
void TesseraGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void TesseraGUI::updateWalletStatus()
{
    assert(walletFrame);
...
void TesseraGUI::updateProxyIcon()
{
    std::string ip_port;
...
void TesseraGUI::updateWindowTitle()
{
...
void TesseraGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void TesseraGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void TesseraGUI::detectShutdown()
{
...
void TesseraGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0) {
...
void TesseraGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

[[nodiscard]] static bool ThreadSafeMessageBox(TesseraGUI* gui, const bilingual_str& message, unsigned int style)
{
    ...
    if (message.original != message.translated) {
        detailed_message = TesseraGUI::tr("Original message:") + "\n" + QString::fromStdString(message.original);
    }
...
void TesseraGUI::subscribeToCoreSignals()
{
...
void TesseraGUI::unsubscribeFromCoreSignals()
{
...
bool TesseraGUI::isPrivacyModeActivated() const
{
    ...
    : m_platform_style{platformStyle}
{
    ...
    QList<TesseraUnit> units = TesseraUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const TesseraUnit unit : units) {
        max_width = qMax(max_width, GUIUtil::TextWidth(fm, TesseraUnits::longName(unit)));
    }
...
{
    menu = new QMenu(this);
    for (const TesseraUnit u : TesseraUnits::availableUnits()) {
        menu->addAction(TesseraUnits::longName(u))->setData(QVariant::fromValue(u));
    }
...
void UnitDisplayStatusBarControl::updateDisplayUnit(TesseraUnit newUnits)
{
    setText(TesseraUnits::longName(newUnits));
}
