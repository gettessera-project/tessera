...
{
    return IsValidDestinationString(address.toStdString(), Params());
}
...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction &transaction, const CCoinControl& coinControl)
{
    ...
    for (const SendCoinsRecipient &rcp : recipients)
    {
        ...
        {   // User-entered tessera address / amount:
            ...
            vecSend.emplace_back(CRecipient{DecodeDestination(rcp.address.toStdString(), Params()), rcp.amount, rcp.fSubtractFeeFromAmount});
...
void WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        {
            if (!rcp.message.isEmpty()) // Message from normal tessera:URI (tessera:123...?message=example)
...
{
    QString strAddress = QString::fromStdString(EncodeDestination(address, Params()));
...
bool WalletModel::bumpFee(Txid hash, Txid& new_hash)
{
    ...
    questionString.append(TesseraUnits::formatHtmlWithUnit(getOptionsModel()->getDisplayUnit(), old_fee));
    questionString.append("</td></tr><tr><td>");
    questionString.append(tr("Increase:"));
    questionString.append("</td><td>");
    questionString.append(TesseraUnits::formatHtmlWithUnit(getOptionsModel()->getDisplayUnit(), new_fee - old_fee));
    questionString.append("</td></tr><tr><td>");
    questionString.append(tr("New fee:"));
    questionString.append("</td><td>");
    questionString.append(TesseraUnits::formatHtmlWithUnit(getOptionsModel()->getDisplayUnit(), new_fee));
    ...
    const bool enable_send{!wallet().privateKeysDisabled()};
...
{
    // Tessera has no external-signer support (interfaces::Wallet::displayAddress
    // is not part of the interface), so there is no device to display on.
    (void)sAddress;
}

bool WalletModel::isWalletEnabled()
{
   return !gArgs.GetBoolArg("-disablewallet", false);
}
