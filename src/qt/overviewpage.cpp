...
public:
                      ...
                      const QModelIndex &index ) const override
    {
        ...
        painter->setPen(foreground);
        QString amountText = TesseraUnits::formatWithUnit(unit, amount, true, TesseraUnits::SeparatorStyle::ALWAYS);
        if(!confirmed)
        {
    ...
    TesseraUnit unit{TesseraUnit::TSR};
    ...
    txdelegate(new TxViewDelegate(platformStyle, this))
{
    ...
    // Draw a tight box around the warning icon ourselves: the native button keeps a
    // large internal padding that leaves grey slack to the right of the 24px triangle.
    // A small palette-based box (theme-adaptive) hugs the icon instead.
    const QString status_style{QStringLiteral(
        "QPushButton { border: 1px solid palette(mid); border-radius: 3px;"
        " background-color: palette(button); padding: 2px; }"
        "QPushButton:hover { background-color: palette(midlight); }")};
    ui->labelTransactionsStatus->setStyleSheet(status_style);
    ui->labelWalletStatus->setStyleSheet(status_style);
...
{
    TesseraUnit unit = walletModel->getOptionsModel()->getDisplayUnit();
    ui->labelBalance->setText(TesseraUnits::formatWithPrivacy(unit, balances.balance, TesseraUnits::SeparatorStyle::ALWAYS, m_privacy));
    ui->labelUnconfirmed->setText(TesseraUnits::formatWithPrivacy(unit, balances.unconfirmed_balance, TesseraUnits::SeparatorStyle::ALWAYS, m_privacy));
    ui->labelImmature->setText(TesseraUnits::formatWithPrivacy(unit, balances.immature_balance, TesseraUnits::SeparatorStyle::ALWAYS, m_privacy));
    ui->labelTotal->setText(TesseraUnits::formatWithPrivacy(unit, balances.balance + balances.unconfirmed_balance + balances.immature_balance, TesseraUnits::SeparatorStyle::ALWAYS, m_privacy));
