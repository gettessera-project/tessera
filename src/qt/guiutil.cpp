...
// Return a pre-generated dummy address with invalid checksum.
static std::string DummyAddress(const CChainParams &params)
{
    std::string addr;
    switch (params.GetChainType()) {
    case ChainType::MAIN:
        addr = "lta1p35yvjel7srp783ztf8v6jdra7dhfzk5jaun8xz2qp6ws7z80n4tq2jku9f";
        break;
    case ChainType::TESTNET:
        addr = "tlta1p35yvjel7srp783ztf8v6jdra7dhfzk5jaun8xz2qp6ws7z80n4tqa6qnlg";
        break;
    case ChainType::REGTEST:
        addr = "ltart1p35yvjel7srp783ztf8v6jdra7dhfzk5jaun8xz2qp6ws7z80n4tqsr2427";
        break;
    } // no default case, so the compiler can warn about missing cases
    assert(!addr.empty());

    if (Assume(!IsValidDestinationString(addr, params))) return addr;
    return {};
}
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Tessera address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new TesseraAddressEntryValidator(parent));
    widget->setCheckValidator(new TesseraAddressCheckValidator(parent));
}
...
bool parseTesseraURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no tessera: URI
    if(!uri.isValid() || uri.scheme() != QString("tessera"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if (!TesseraUnits::parse(TesseraUnit::TSR, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
bool parseTesseraURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseTesseraURI(uriInstance, out);
}

QString formatTesseraURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("tessera:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(TesseraUnits::format(TesseraUnit::TSR, info.amount, false, TesseraUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
{
    CTxDestination dest = DecodeDestination(address.toStdString(), Params());
...
bool openTesseraConf()
{
    ...
    /* Open tessera.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == ChainType::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Tessera.lnk";
    if (chain == ChainType::TESTNET) // Remove this special case when testnet CBaseChainParams::DataDir() is incremented to "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Tessera (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / fs::u8path(strprintf("Tessera (%s).lnk", ChainTypeToString(chain)));
}

bool GetStartOnSystemStartup()
{
    // check for Tessera*.lnk
...
fs::path static GetAutostartFilePath()
{
    ChainType chain = gArgs.GetChainType();
    if (chain == ChainType::MAIN)
        return GetAutostartDir() / "tessera.desktop";
    return GetAutostartDir() / fs::u8path(strprintf("tessera-%s.desktop", ChainTypeToString(chain)));
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        ChainType chain = gArgs.GetChainType();
        // Write a tessera.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == ChainType::MAIN)
            optionFile << "Name=Tessera\n";
        else
            optionFile << strprintf("Name=Tessera (%s)\n", ChainTypeToString(chain));
