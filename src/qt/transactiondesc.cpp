...
QString TransactionDesc::toHTML(interfaces::Node& node, interfaces::Wallet& wallet, TransactionRecord* rec, TesseraUnit unit)
{
    int numBlocks;
    ...
    else
    {
        ...
        {
            // Credit
            CTxDestination address = DecodeDestination(rec->address, Params());
    ...
    if (wtx.is_coinbase && nCredit == 0)
    {
        ...
        if (status.is_in_main_chain)
            strHTML += TesseraUnits::formatHtmlWithUnit(unit, nUnmatured)+ " (" + tr("matures in %n more block(s)", "", status.blocks_to_maturity) + ")";
        else
    ...
    else if (nNet > 0)
    {
        ...
        // Credit
        //
        strHTML += "<b>" + tr("Credit") + ":</b> " + TesseraUnits::formatHtmlWithUnit(unit, nNet) + "<br>";
    }
    else
    {
        ...
        if (all_from_me)
        {
            ...
            for (const CTxOut& txout : wtx.tx->vout)
            {
                ...
                if (!wtx.value_map.contains("to") || wtx.value_map["to"].empty())
                {
                    ...
                    if (ExtractDestination(txout.scriptPubKey, address))
                    {
                        ...
                        if (wallet.getAddress(
                            ...
                            strHTML += GUIUtil::HtmlEscape(name) + " ";
                        strHTML += GUIUtil::HtmlEscape(EncodeDestination(address, Params()));
                        if(toSelf)
                ...
                strHTML += "<b>" + tr("Debit") + ":</b> " + TesseraUnits::formatHtmlWithUnit(unit, -txout.nValue) + "<br>";
                if(toSelf)
                    strHTML += "<b>" + tr("Credit") + ":</b> " + TesseraUnits::formatHtmlWithUnit(unit, txout.nValue) + "<br>";
            }
            ...
            if (all_to_me)
            {
                ...
                strHTML += "<b>" + tr("Total debit") + ":</b> " + TesseraUnits::formatHtmlWithUnit(unit, -nValue) + "<br>";
                strHTML += "<b>" + tr("Total credit") + ":</b> " + TesseraUnits::formatHtmlWithUnit(unit, nValue) + "<br>";
            }

            CAmount nTxFee = nDebit - wtx.tx->GetValueOut();
            if (nTxFee > 0)
                strHTML += "<b>" + tr("Transaction fee") + ":</b> " + TesseraUnits::formatHtmlWithUnit(unit, -nTxFee) + "<br>";
        }
        else
        {
            //
            ...
            for (const CTxIn& txin : wtx.tx->vin) {
                if (*(mine++)) {
                    strHTML += "<b>" + tr("Debit") + ":</b> " + TesseraUnits::formatHtmlWithUnit(unit, -wallet.getDebit(txin)) + "<br>";
                }
            }
            mine = wtx.txout_is_mine.begin();
            for (const CTxOut& txout : wtx.tx->vout) {
                if (*(mine++)) {
                    strHTML += "<b>" + tr("Credit") + ":</b> " + TesseraUnits::formatHtmlWithUnit(unit, wallet.getCredit(txout)) + "<br>";
                }
            }
        }
    }

    strHTML += "<b>" + tr("Net amount") + ":</b> " + TesseraUnits::formatHtmlWithUnit(unit, nNet, true) + "<br>";
    ...
    // Message from normal tessera:URI (tessera:123...?message=example)
            ...
            if (auto prev{node.getUnspentOutput(prevout)}) {
                {
                    ...
                    if (ExtractDestination(vout.scriptPubKey, address))
                    {
                        ...
                        if (wallet.getAddress(address, &name, /*purpose=*/nullptr) && !name.empty())
                            strHTML += GUIUtil::HtmlEscape(name) + " ";
                        strHTML += QString::fromStdString(EncodeDestination(address, Params()));
                    }
                    strHTML = strHTML + " " + tr("Amount") + "=" + TesseraUnits::formatHtmlWithUnit(unit, vout.nValue);
