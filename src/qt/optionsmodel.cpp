...
//! Convert enabled/size values to tessera -prune setting.
...
//! Get pruning enabled value to show in GUI from tessera -prune setting.
...
//! Get pruning size value to show in GUI from tessera -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayTesseraUnit")) {
        settings.setValue("DisplayTesseraUnit", QVariant::fromValue(TesseraUnit::TSR));
    }
    QVariant unit = settings.value("DisplayTesseraUnit");
    if (unit.canConvert<TesseraUnit>()) {
        m_display_tessera_unit = unit.value<TesseraUnit>();
    } else {
        m_display_tessera_unit = TesseraUnit::TSR;
        settings.setValue("DisplayTesseraUnit", QVariant::fromValue(m_display_tessera_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if tessera.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_tessera_unit);
...
{
    if (new_unit.isNull() || new_unit.value<TesseraUnit>() == m_display_tessera_unit) return;
    m_display_tessera_unit = new_unit.value<TesseraUnit>();
    QSettings settings;
    settings.setValue("DisplayTesseraUnit", QVariant::fromValue(m_display_tessera_unit));
    Q_EMIT displayUnitChanged(m_display_tessera_unit);
}
