...
CPubKey HexToPubKey(const std::string& hex_in)
{
    ...
    // Tessera public keys are ML-DSA, a fixed CPubKey::SIZE bytes (not 33/65-byte EC points).
    if (hex_in.length() != CPubKey::SIZE * 2) {
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, strprintf("Pubkey \"%s\" must have a length of %d bytes", hex_in, CPubKey::SIZE));
    }
...
CTxDestination AddAndGetMultisigDestination(const int required, const std::vector<CPubKey>& pubkeys, OutputType type, FlatSigningProvider& keystore, CScript& script_out)
{
    ...
    // Tessera keys are ML-DSA with no compressed/uncompressed distinction, so there is no
    // legacy (uncompressed-key) output type; multisig is witness-only.
...
public:
    ...
    // Tessera has no Taproot, so there is no WitnessV1Taproot destination to describe.
