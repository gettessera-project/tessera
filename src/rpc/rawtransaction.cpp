        ...
        [](const RPCMethod& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    if (can_wrap) {
        r.pushKV("p2sh", EncodeDestination(ScriptHash(script), Params()));
        ...
        if (can_wrap_P2WSH) {
            ...
            sr.pushKV("p2sh-segwit", EncodeDestination(ScriptHash(segwitScr), Params()));
        ...
        [](const RPCMethod& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    for (unsigned int i = 0; i < mergedTx.vin.size(); i++) {
        ...
        ProduceSignature(DUMMY_SIGNING_PROVIDER, MutableTransactionSignatureCreator(mergedTx, i, coin.out.nValue, SIGHASH_ALL), coin.out.scriptPubKey, sigdata);
