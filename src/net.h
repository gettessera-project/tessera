...
#ifndef TESSERA_NET_H
#define TESSERA_NET_H
 ...
 *
 * The largest message we ever send is `block`, whose nMessageSize is exactly the serialized
 * block — the message envelope is not counted here (the v2 transport adds its own 1-byte type
 * marker and 12-byte message type on top of this limit, see V2Transport::ProcessReceivedPacketBytes).
 * So this tracks MAX_BLOCK_SERIALIZED_SIZE exactly: any lower and a consensus-valid block becomes
 * undeliverable — the peer drops the packet, disconnects, and every node behind the first oversized
 * block stalls while the miner extends its own tip. V2Cipher::LENGTH_LEN is 4 bytes so a packet
 * this size can be described on the wire at all.
 *
 * Raising this does not reintroduce the receive-buffer OOM of
 * https://bitcoincore.org/en/2024/07/03/disclose_receive_buffer_oom: both transports grow the
 * receive buffer incrementally (256 KiB beyond the bytes actually delivered, see
 * V1Transport::readData and V2Transport::ReceivedBytes), never eagerly to the announced length,
 * so a peer cannot claim memory it has not already paid for in bandwidth. */
static const unsigned int MAX_PROTOCOL_MESSAGE_LENGTH = MAX_BLOCK_SERIALIZED_SIZE;
// nMessageSize is bounded by MAX_SIZE as well, so this has to fit under it to be the binding limit.
static_assert(MAX_PROTOCOL_MESSAGE_LENGTH <= MAX_SIZE,
              "MAX_PROTOCOL_MESSAGE_LENGTH must stay under MAX_SIZE to bind");
...
public:
    ...
    /** v2 transport session id string in hex, if any. */
...
private:
     ...
     *  V2Cipher::LENGTH_LEN. Unspecified otherwise. */
...
public:
    ...
    /** Construct a V2 transport with specified garbage (test use only). The ephemeral
     *  ML-KEM keypair is always generated internally, so only the garbage is injectable. */
    V2Transport(NodeId nodeid, bool initiating, std::vector<uint8_t> garbage) noexcept;
