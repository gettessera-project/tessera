...
/** Helper to easily feed data into a SHA3_512.
 *
 ...
 */
template<typename T>
SHA3_512& operator<<(SHA3_512& hasher, const T& data) {
    static_assert(!std::is_same_v<std::decay_t<T>, char*>, "Calling operator<<(SHA3_512, char*) is probably not what you want");
    static_assert(!std::is_same_v<std::decay_t<T>, unsigned char*>, "Calling operator<<(SHA3_512, unsigned char*) is probably not what you want");
    static_assert(!std::is_same_v<std::decay_t<T>, const char*>, "Calling operator<<(SHA3_512, const char*) is probably not what you want");
    static_assert(!std::is_same_v<std::decay_t<T>, const unsigned char*>, "Calling operator<<(SHA3_512, const unsigned char*) is probably not what you want");
    hasher.Write((const unsigned char*)&data, sizeof(data));
    return hasher;
}

#ifndef WIN32
void AddSockaddr(SHA3_512& hasher, const struct sockaddr *addr)
{
...
void AddFile(SHA3_512& hasher, const char *path)
{
    struct stat sb = {};
...
void AddPath(SHA3_512& hasher, const char *path)
{
    struct stat sb = {};
    if (stat(path, &sb) == 0) {
...
#ifdef HAVE_SYSCTL
template<int... S>
void AddSysctl(SHA3_512& hasher)
{
    int CTL[sizeof...(S)] = {S...};
...
#ifdef HAVE_GETCPUID
void inline AddCPUID(SHA3_512& hasher, uint32_t leaf, uint32_t subleaf, uint32_t& ax, uint32_t& bx, uint32_t& cx, uint32_t& dx)
{
    GetCPUID(leaf, subleaf, ax, bx, cx, dx);
    hasher << leaf << subleaf << ax << bx << cx << dx;
}

void AddAllCPUID(SHA3_512& hasher)
{
