...
const char * const BITCOIN_CONF_FILENAME = "tessera.conf";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //bTessera-tx using stdin
...
{
    // Windows:
    //   old: C:\Users\Username\AppData\Roaming\BTessera
    //   new: C:\Users\Username\AppData\Local\BTessera
    // macOS: ~/Library/Application Support/Tessera
    // Unix-like: ~/.tessera
#ifdef WIN32
    // Windows
    // Check for existence of datadir in old location and keep it there
    fs::path legacy_path = GetSpecialFolderPath(CSIDL_APPDATA) / "Tessera";
    if (fs::exists(legacy_path)) return legacy_path;

    // Otherwise, fresh installs can start in the new, "proper" location
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "Tessera";
#else
    fs::path pathRet;
...
#ifdef __APPLE__
    // macOS
    return pathRet / "Library/Application Support/Tessera";
#else
    // Unix-like
    return pathRet / ".tessera";
#endif
#endif
}
