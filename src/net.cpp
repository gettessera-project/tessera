...
/** List of short messages for the v2 transport, in order (the same short-id scheme as BIP324).
 *
 ...
 */
constexpr size_t V2_SHORTIDS_IMPLEMENTED{38};
const std::array<std::string, V2_SHORTIDS_IMPLEMENTED> V2_MESSAGE_IDS = {
...
void V2Transport::StartSendingHandshake() noexcept
{
    ...
    // The responder's key-agreement message is the ML-KEM ciphertext, which can only be produced
    // once the initiator's encapsulation key has arrived; the responder therefore leaves its send
    // buffer empty here and fills it in ProcessReceivedKeyBytes. Only the initiator, whose key
    // share (the encapsulation key) is ready at construction, starts sending immediately.
    if (!m_initiating) return;
    // Initialize the send buffer with our encapsulation key + provided garbage.
    const auto key_share = m_cipher.GetOurKeyShare();
    m_send_buffer.resize(key_share.size() + m_send_garbage.size());
    std::copy(key_share.begin(), key_share.end(), MakeWritableByteSpan(m_send_buffer).begin());
    std::copy(m_send_garbage.begin(), m_send_garbage.end(), m_send_buffer.begin() + key_share.size());
    // We cannot wipe m_send_garbage as it will still be used as AAD later in the handshake.
}

V2Transport::V2Transport(NodeId nodeid, bool initiating, std::vector<uint8_t> garbage) noexcept
    : m_cipher{initiating, MakeByteSpan(Params().MessageStart())},
...
V2Transport::V2Transport(NodeId nodeid, bool initiating) noexcept
    : V2Transport{nodeid, initiating, GenerateRandomGarbage()} {}
...
bool V2Transport::ProcessReceivedKeyBytes() noexcept
{
    ...
    // The initiator receives the responder's ML-KEM ciphertext; the responder receives the
    // initiator's encapsulation key. The two messages differ in size, so the expected length is
    // role-dependent.
    const size_t expected_size = m_initiating ? V2Cipher::CT_LEN : V2Cipher::EK_LEN;
    Assume(m_recv_buffer.size() <= expected_size);
    ...
    if (m_recv_buffer.size() == expected_size) {
        // The peer's key-agreement message has been fully received; complete ML-KEM key agreement
        // (the initiator decapsulates the ciphertext, the responder encapsulates to the key) to
        // initialize the encryption ciphers.
        LOCK(m_send_mutex);
        if (!m_cipher.Initialize(MakeByteSpan(m_recv_buffer))) return false;
        ...
        // The responder could not send its key share until now: its ciphertext is produced by the
        // encapsulation just performed in Initialize(). Place it, followed by our garbage, in the
        // send buffer ahead of the garbage terminator (the initiator already sent ek + garbage).
        if (!m_initiating) {
            const auto key_share = m_cipher.GetOurKeyShare();
            const size_t base = m_send_buffer.size();
            m_send_buffer.resize(base + key_share.size() + m_send_garbage.size());
            std::copy(key_share.begin(), key_share.end(), MakeWritableByteSpan(m_send_buffer).subspan(base).begin());
            std::copy(m_send_garbage.begin(), m_send_garbage.end(), m_send_buffer.begin() + base + key_share.size());
        }

        // Append the garbage terminator to the send buffer.
        m_send_buffer.resize(m_send_buffer.size() + V2Cipher::GARBAGE_TERMINATOR_LEN);
        std::copy(m_cipher.GetSendGarbageTerminator().begin(),
                  m_cipher.GetSendGarbageTerminator().end(),
                  MakeWritableByteSpan(m_send_buffer).last(V2Cipher::GARBAGE_TERMINATOR_LEN).begin());

        // Construct version packet in the send buffer, with the sent garbage data as AAD.
        m_send_buffer.resize(m_send_buffer.size() + V2Cipher::EXPANSION + VERSION_CONTENTS.size());
        m_cipher.Encrypt(
            /*contents=*/VERSION_CONTENTS,
            /*aad=*/MakeByteSpan(m_send_garbage),
            /*ignore=*/false,
            /*output=*/MakeWritableByteSpan(m_send_buffer).last(V2Cipher::EXPANSION + VERSION_CONTENTS.size()));
...
bool V2Transport::ProcessReceivedGarbageBytes() noexcept
{
    ...
    Assume(m_recv_buffer.size() <= MAX_GARBAGE_LEN + V2Cipher::GARBAGE_TERMINATOR_LEN);
    if (m_recv_buffer.size() >= V2Cipher::GARBAGE_TERMINATOR_LEN) {
        if (std::ranges::equal(MakeByteSpan(m_recv_buffer).last(V2Cipher::GARBAGE_TERMINATOR_LEN), m_cipher.GetReceiveGarbageTerminator())) {
            // Garbage terminator received. Store garbage to authenticate it as AAD later.
            m_recv_aad = std::move(m_recv_buffer);
            m_recv_aad.resize(m_recv_aad.size() - V2Cipher::GARBAGE_TERMINATOR_LEN);
            m_recv_buffer.clear();
            SetReceiveState(RecvState::VERSION);
        } else if (m_recv_buffer.size() == MAX_GARBAGE_LEN + V2Cipher::GARBAGE_TERMINATOR_LEN) {
...
bool V2Transport::ProcessReceivedPacketBytes() noexcept
{
    ...
    if (m_recv_buffer.size() == V2Cipher::LENGTH_LEN) {
    ...
    } else if (m_recv_buffer.size() > V2Cipher::LENGTH_LEN && m_recv_buffer.size() == m_recv_len + V2Cipher::EXPANSION) {
        ...
        bool ret = m_cipher.Decrypt(
            /*input=*/MakeByteSpan(m_recv_buffer).subspan(V2Cipher::LENGTH_LEN),
...
size_t V2Transport::GetMaxBytesToProcess() noexcept
{
    ...
    case RecvState::KEY: {
        // During the KEY state, we only allow the peer's key-agreement message into the receive
        // buffer: the responder's ML-KEM ciphertext for an initiator, the initiator's
        // encapsulation key for a responder.
        const size_t expected_size = m_initiating ? V2Cipher::CT_LEN : V2Cipher::EK_LEN;
        Assume(m_recv_buffer.size() <= expected_size);
        // As long as we have not received the other side's key share, don't receive more than
        // that, as garbage follows, and locating the garbage terminator requires the key
        // exchange first.
        return expected_size - m_recv_buffer.size();
    }
    case RecvState::GARB_GARBTERM:
    ...
    case RecvState::APP:
        ...
        if (m_recv_buffer.size() < V2Cipher::LENGTH_LEN) {
            return V2Cipher::LENGTH_LEN - m_recv_buffer.size();
        } else {
            // Note that V2Cipher::EXPANSION is the total difference between contents size
            ...
            return V2Cipher::EXPANSION + m_recv_len - m_recv_buffer.size();
        }
    case RecvState::APP_READY:
    ...
    while (!msg_bytes.empty()) {
        ...
        if (m_recv_buffer.size() + std::min(msg_bytes.size(), max_read) > m_recv_buffer.capacity()) {
            ...
            case RecvState::GARB_GARBTERM:
                ...
                // bytes).
                m_recv_buffer.reserve(MAX_GARBAGE_LEN + V2Cipher::GARBAGE_TERMINATOR_LEN);
                break;
            case RecvState::VERSION:
            case RecvState::APP: {
...
CNetMessage V2Transport::GetReceivedMessage(NodeClock::time_point time, bool& reject_message) noexcept
{
    ...
    // Note that V2Cipher::EXPANSION also includes the length descriptor size.
    msg.m_raw_message_size = m_recv_decode_buffer.size() + V2Cipher::EXPANSION;
    if (msg_type) {
        reject_message = false;
...
bool V2Transport::SetMessageToSend(CSerializedNetMsg& msg) noexcept
{
    ...
    m_send_buffer.resize(contents.size() + V2Cipher::EXPANSION);
            ...
            if (HaveNameProxy()) {
                ...
                if (!addresses.empty()) {
                    for (const CNetAddr& ip : addresses) {
                        // requiredServiceBits stays in the query string above -- that
                        // is what the seed filters on. What we record locally is what
                        // we assume of the peer, which includes v2 transport.
