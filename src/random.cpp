...
void SeedHardwareFast(SHA3_512& hasher) noexcept {
...
void SeedHardwareSlow(SHA3_512& hasher) noexcept {
#if defined(__x86_64__) || defined(__amd64__) || defined(__i386__)
...
void Strengthen(const unsigned char (&seed)[32], SteadyClock::duration dur, SHA3_512& hasher) noexcept
{
    SHA3_512 inner_hasher;
...
class RNGState {
    ...
    SHA3_512 m_events_hasher GUARDED_BY(m_events_mutex);
...
public:
     ...
     */
    void SeedEvents(SHA3_512& hasher) noexcept EXCLUSIVE_LOCKS_REQUIRED(!m_events_mutex)
    {
        // Tessera hashes events with SHA3-512 too (the RNG is uniformly SHA3-512);
        // this just needs to be fast as network peers may be able to trigger it
        // repeatedly.
        LOCK(m_events_mutex);

        unsigned char events_hash[SHA3_512::OUTPUT_SIZE];
     ...
     */
    bool MixExtract(unsigned char* out, size_t num, SHA3_512&& hasher, bool strong_seed, bool always_use_real_rng) noexcept EXCLUSIVE_LOCKS_REQUIRED(!m_mutex)
    {
        assert(num <= 32);
        unsigned char buf[64];
        static_assert(sizeof(buf) == SHA3_512::OUTPUT_SIZE, "Buffer needs to have hasher's output size");
        bool ret;
        {
            LOCK(m_mutex);
...
void SeedTimestamp(SHA3_512& hasher) noexcept
{
    int64_t perfcounter = GetPerformanceCounter();
    hasher.Write((const unsigned char*)&perfcounter, sizeof(perfcounter));
}

void SeedFast(SHA3_512& hasher) noexcept
{
...
void SeedSlow(SHA3_512& hasher, RNGState& rng) noexcept
{
    unsigned char buffer[32];
...
void SeedStrengthen(SHA3_512& hasher, RNGState& rng, SteadyClock::duration dur) noexcept
{
    // Generate 32 bytes of entropy from the RNG, and a copy of the entropy already in hasher.
    // Never use the deterministic PRNG for this, as the result is only used internally.
    unsigned char strengthen_seed[32];
    rng.MixExtract(strengthen_seed, sizeof(strengthen_seed), SHA3_512(hasher), false, /*always_use_real_rng=*/true);
    // Strengthen the seed, and feed it into hasher.
    Strengthen(strengthen_seed, dur, hasher);
}

void SeedPeriodic(SHA3_512& hasher, RNGState& rng) noexcept
{
    // Everything that the 'fast' seeder includes
    SeedFast(hasher);
...
void SeedStartup(SHA3_512& hasher, RNGState& rng) noexcept
{
