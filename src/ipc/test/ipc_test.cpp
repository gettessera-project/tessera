...
static std::string TempPath(std::string_view pattern)
{
    ...
    IpcCheck("mkstemp fd >= 0", fd >= 0);
    IpcCheck("close temp fd", close(fd) == 0);
...
void IpcPipeTest()
{
    ...
    IpcCheck("passTransaction roundtrip", *Assert(tx1) == *Assert(tx2));

    std::vector<char> vec1{'H', 'e', 'l', 'l', 'o'};
    std::vector<char> vec2{foo->passVectorChar(vec1)};
    IpcCheck("passVectorChar roundtrip",
             std::string_view(vec1.begin(), vec1.end()) == std::string_view(vec2.begin(), vec2.end()));

    auto script1{CScript() << OP_11};
    auto script2{foo->passScript(script1)};
    IpcCheck("passScript roundtrip", HexStr(script1) == HexStr(script2));
...
{
    int fds[2];
    IpcCheck("socketpair ok", socketpair(AF_UNIX, SOCK_STREAM, 0, fds) == 0);
    ...
    IpcCheck("socketpair echo roundtrip", remote_echo->echo("echo test") == "echo test");
    remote_echo.reset();
    remote_init.reset();
    thread.join();
}
...
void IpcSocketTest(const fs::path& datadir)
{
    ...
    IpcCheck("bind invalid throws", throws_invalid_argument([&] { process->bind(datadir, "test_tessera", invalid_bind); }));
    IpcCheck("connect invalid throws", throws_invalid_argument([&] { process->connect(datadir, "test_tessera", invalid_bind); }));

    auto bind_and_listen{[&](const std::string& bind_address) {
        std::string address{bind_address};
        int serve_fd = process->bind(datadir, "test_tessera", address);
        IpcCheck("bind fd >= 0", serve_fd >= 0);
        IpcCheck("bind address unchanged", address == bind_address);
    ...
    auto connect_and_test{[&](const std::string& connect_address) {
        ...
        int connect_fd{process->connect(datadir, "test_tessera", address)};
        IpcCheck("connect address unchanged", address == connect_address);
        std::unique_ptr<interfaces::Init> remote_init{protocol->connect(connect_fd, "test-connect")};
        std::unique_ptr<interfaces::Echo> remote_echo{remote_init->makeEcho()};
        IpcCheck("socket echo roundtrip", remote_echo->echo("echo test") == "echo test");
    }};

    // Need to specify explicit socket addresses outside the data directory, because the data
    // directory path may be long enough that the default socket address and any other
    // addresses in the data directory would exceed the maximum socket path length.
    std::vector<std::string> addresses{
        strprintf("unix:%s", TempPath("tessera_sock0_XXXXXX")),
        strprintf("unix:%s", TempPath("tessera_sock1_XXXXXX")),
    };
