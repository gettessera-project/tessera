...
#ifndef TESSERA_UTIL_TRANSLATION_H
#define TESSERA_UTIL_TRANSLATION_H
...
// The MSVC C/C++ IntelliSense parser does not apply the consteval BilingualFmt converting
// constructor during overload resolution, so it spuriously rejects strprintf(_("…"), …).
// Give it a direct exact match. __INTELLISENSE__ is defined only while IntelliSense parses;
// the real toolchain never sees this overload and keeps the compile-time format validation
// performed by the BilingualFmt overload below.
template <typename... Args>
bilingual_str format(const util::TranslatedLiteral& fmt, const Args&... /*args*/)
{
    return bilingual_str{fmt.original, fmt.original};
}
#endif
template <typename... Args>
...
#endif // TESSERA_UTIL_TRANSLATION_H
