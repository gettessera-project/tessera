...
#ifndef TESSERA_UTIL_STRING_H
#define TESSERA_UTIL_STRING_H
...
struct LineReader {
    ...
    explicit LineReader(std::span<const std::byte> buffer, size_t max_line_length_);
    explicit LineReader(std::string_view str, size_t max_line_length_) : LineReader{std::as_bytes(std::span{str}), max_line_length_} {}
