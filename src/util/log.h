...
#ifndef TESSERA_UTIL_LOG_H
#define TESSERA_UTIL_LOG_H
...
// IntelliSense cannot construct a consteval ConstevalFormatString from a string
// literal at parse time, so it fails to match these overloads at every LogInfo/
// LogError/... call site. Give it plain const char* stubs; real compilation uses
// the consteval-checked templates in the #else branch below (which the actual
// gcc build compiles, format string verified at compile time).
template <typename... Args>
inline void LogPrintFormatInternal(SourceLocation&&, BCLog::LogFlags, util::log::Level, const char*, const Args&...) {}
template <typename... Args>
inline void LogPrintFormatInternal(SourceLocation&&, BCLog::LogFlags, util::log::Level, util::log::NoRateLimitTag, const char*, const Args&...) {}
#else
template <typename... Args>
