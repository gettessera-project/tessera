...
#include <algorithm>
#include <attributes.h> // Added for Tessera
#include <iostream>
#include <sstream>
#include <stdexcept> // Added for Tessera
#include <util/string.h> // Added for Tessera
...
// Added for Tessera. Similar to std::runtime_format from C++26.
...
// Added for Tessera. Wrapper for checking format strings at compile time.
...
// IDE shim (not compiled): the MS C/C++ extension's IntelliSense parser cannot
// evaluate the consteval FormatStringCheck implicit conversion during overload
// resolution, so it falsely flags every strprintf()/tfm::format() call as "no
// matching overload". (g++ compiles them fine, with the compile-time format-arg
// check intact.) These plain const char* declarations give IntelliSense an
// overload it can resolve — string-literal decay outranks the consteval
// user-defined conversion, so there's no ambiguity. __INTELLISENSE__ is defined
// only while IntelliSense parses; the real compiler never sees these, so Core's
// consteval format-string checking below is preserved in actual builds.
