...
{
    // According to BIP-330, salts should be combined in ascending order. This is a P2P short-id
    // seed (not a consensus value), so Tessera derives it with its SHA3-256 HashWriter, seeded
    // with the static salt tag for domain separation.
    HashWriter hasher;
    hasher.write(MakeByteSpan(RECON_STATIC_SALT));
    hasher << std::min(salt1, salt2) << std::max(salt1, salt2);
