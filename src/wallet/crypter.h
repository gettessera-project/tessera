...
#ifndef TESSERA_WALLET_CRYPTER_H
#define TESSERA_WALLET_CRYPTER_H
 ...
 * (0 == SHA3-512, Tessera's Keccak-only stand-in for Core's EVP_sha512())
 * and derivation iterations nDeriveIterations.
 * vchOtherDerivationParameters is provided for alternative algorithms
 * which may require more parameters.
 *
 * Wallet Private Keys are then encrypted using AES-256-CBC
 * with the hash (SHA3-256) of the public key as the IV, and the
...
public:
    ...
    //! 0 = SHA3-512 (Tessera); Core uses EVP_sha512()
...
private:
    ...
    int BytesToKeySHA3_512AES(std::span<const unsigned char> salt, const SecureString& key_data, int count, unsigned char* key, unsigned char* iv) const;
...
#endif // TESSERA_WALLET_CRYPTER_H
