...
//! Tessera always compiles the wallet in; these two settings have no dedicated
//! constant in the reduced wallet headers, so they are spelled out here.
static constexpr bool DEFAULT_DISABLE_WALLET{false};
static constexpr bool DEFAULT_WALLETCROSSCHAIN{false};
...
void WalletInit::AddWalletOptions(ArgsManager& argsman) const
{
    ...
    argsman.AddArg("-avoidpartialspends", strprintf("Group outputs by address, selecting many (possibly all) or none, instead of selecting on a per-output basis. Always enabled for wallets with \"avoid_reuse\" enabled, otherwise default: %u.", DEFAULT_AVOIDPARTIALSPENDS), ArgsManager::ALLOW_ANY, OptionsCategory::WALLET);
    argsman.AddArg("-changetype",
                   strprintf("What type of change to use (%s).", FormatAllOutputTypes()),
    ...
    argsman.AddArg("-wallet=<path>", "Specify wallet path to load at startup. Can be used multiple times to load multiple wallets. Path is to a directory containing wallet data and log files. If the path is not absolute, it is interpreted relative to <walletdir>.", ArgsManager::ALLOW_ANY | ArgsManager::NETWORK_ONLY, OptionsCategory::WALLET);
