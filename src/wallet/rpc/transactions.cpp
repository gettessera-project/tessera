...
namespace wallet {
//! Push the "address" field for a destination, if it has one.
static void MaybePushAddress(UniValue& entry, const CTxDestination& dest)
{
    if (IsValidDestination(dest)) {
        entry.pushKV("address", EncodeDestination(dest, Params()));
    }
}
    ...
    EXCLUSIVE_LOCKS_REQUIRED(wallet.cs_wallet)
{
    ...
    // The legacy bip125-replaceable field is omitted: Tessera's interfaces::Chain
    // does not expose rpcEnableDeprecated / isRBFOptIn (the field was deprecated in
    // Core). RBF itself is supported; it is simply not surfaced here.
...
static UniValue ListReceived(const CWallet& wallet, const UniValue& params, const bool by_label, const bool include_immature_coinbase) EXCLUSIVE_LOCKS_REQUIRED(wallet.cs_wallet)
{
    ...
    if (!by_label && !params[3].isNull() && !params[3].get_str().empty()) {
        if (!IsValidDestinationString(params[3].get_str(), Params())) {
            throw JSONRPCError(RPC_WALLET_ERROR, "address_filter parameter was invalid");
        }
        filtered_address = DecodeDestination(params[3].get_str(), Params());
    }
    ...
    const auto& func = [&](const CTxDestination& address, const std::string& label, bool is_change, const std::optional<AddressPurpose>& purpose) {
        ...
        if (by_label) {
            ...
            obj.pushKV("address",       EncodeDestination(address, Params()));
    ...
    EXCLUSIVE_LOCKS_REQUIRED(wallet.cs_wallet)
{
    ...
    if (listReceived.size() > 0 && wallet.GetTxDepthInMainChain(wtx) >= nMinDepth) {
        for (const COutputEntry& r : listReceived)
        {
            ...
            // Tessera has no output descriptors, so no parent_descs are pushed.
            if (wtx.IsCoinBase())
            {
...
RPCMethod listtransactions()
{
    return RPCMethod{
        ...
        "If a label name is provided, this will return only incoming transactions paying to addresses with the specified label.\n"
                ...
                "transactions specified in the 'skip' argument.\n",
                {
                ...
                RPCResult{
                    RPCResult::Type::ARR, "", "",
                    {
                        ...
                        {
                            {RPCResult::Type::STR, "address",  /*optional=*/true, "The tessera address of the transaction (not returned if the output does not have an address, e.g. OP_RETURN null data)."},
        ...
        [](const RPCMethod& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    std::vector<UniValue> ret;
    {
        ...
        // Tessera's CWallet does not maintain an ordered tx index (Core's
        // wtxOrdered); build one on the fly, keyed by nOrderPos.
        std::multimap<int64_t, const CWalletTx*> txOrdered;
        for (const auto& [_, wtx] : pwallet->mapWallet) {
            txOrdered.emplace(wtx.nOrderPos, &wtx);
        }
                ...
                RPCResult{
                    ...
                    TransactionDescriptionString()),
                    {
                        {RPCResult::Type::ARR, "details", "",
                        {
                            {RPCResult::Type::OBJ, "", "",
                            {
                                {RPCResult::Type::STR, "address", /*optional=*/true, "The tessera address involved in the transaction."},
                                {RPCResult::Type::STR, "category", "The transaction category.\n"
                                    "\"send\"                  Transactions sent.\n"
                                    "\"receive\"               Non-coinbase transactions received.\n"
                                    "\"generate\"              Coinbase transactions received with more than 100 confirmations.\n"
