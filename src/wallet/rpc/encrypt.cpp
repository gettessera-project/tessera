...
RPCMethod walletpassphrase()
{
    return RPCMethod{
        ...
        "Stores the wallet decryption key in memory for 'timeout' seconds.\n"
                "This is needed prior to performing transactions related to private keys such as sending tesseras\n"
            "\nNote:\n"
...
RPCMethod encryptwallet()
{
    return RPCMethod{
        ...
        "will require the passphrase to be set prior to making these calls.\n"
            ...
            "\nNow set the passphrase to use the wallet, such as for signing or sending tessera\n"
        ...
        [](const RPCMethod& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    // Tessera seals the existing HD seed in place (the whole key tree re-derives
    // from it), so unlike Core there is no seed rotation or keypool flush and the
    // pre-encryption backup remains valid. Encourage a fresh backup regardless.
    return "wallet encrypted; You should make a new backup of the wallet file with the backupwallet RPC.";
},
    };
}
} // namespace wallet
