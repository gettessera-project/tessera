...
RPCMethod getreceivedbyaddress()
{
    return RPCMethod{
        ...
        "Returns the total amount received by the given address in transactions with at least minconf confirmations.\n",
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The tessera address for transactions."},
...
RPCMethod lockunspent()
{
    return RPCMethod{
        ...
        "Updates list of temporarily unspendable outputs.\n"
                ...
                "A locked transaction output will not be chosen by automatic coin selection, when spending tesseras.\n"
    ...
    return RPCMethod{
        ...
        "Returns array of unspent transaction outputs\n"
                ...
                "Optionally filter to only include txouts paid to specified addresses.\n",
                {
                    ...
                    {"addresses", RPCArg::Type::ARR, RPCArg::Default{UniValue::VARR}, "The tessera addresses to filter",
                        {
                            {"address", RPCArg::Type::STR, RPCArg::Optional::OMITTED, "tessera address"},
                        },
                    },
                ...
                RPCResult{
                    RPCResult::Type::ARR, "", "",
                    {
                        {RPCResult::Type::OBJ, "", "",
                        {
                            ...
                            {RPCResult::Type::NUM, "vout", "the vout value"},
                            {RPCResult::Type::STR, "address", /*optional=*/true, "the tessera address"},
        ...
        [](const RPCMethod& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    if (!request.params[2].isNull()) {
        ...
        for (unsigned int idx = 0; idx < inputs.size(); idx++) {
            ...
            CTxDestination dest = DecodeDestination(input.get_str(), Params());
            if (!IsValidDestination(dest)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Tessera address: ") + input.get_str());
            }
    ...
    for (const COutput& out : vecOutputs) {
        ...
        if (fValidAddress) {
            entry.pushKV("address", EncodeDestination(address, Params()));

            const auto* address_book_entry = pwallet->FindAddressBookEntry(address);
            if (address_book_entry) {
                entry.pushKV("label", address_book_entry->GetLabel());
            }
            // Tessera is witness-v0 (P2WPKH) only: there are no P2SH redeemScripts
            // or P2WSH witnessScripts to surface, and no output descriptors.
        }
