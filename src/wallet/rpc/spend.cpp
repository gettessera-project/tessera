...
// Tessera signs transactions in-wallet with ML-DSA; there is no PSBT / external
// signer path, so Core's FinishTransaction (FillPSBT + FinalizeAndExtractPSBT) is
// reduced to anti-fee-snipe + direct CWallet::SignTransaction.
static UniValue FinishTransaction(const std::shared_ptr<CWallet> pwallet, const UniValue& options, CMutableTransaction& rawTx)
{
    ...
    bool complete = pwallet->SignTransaction(rawTx);

    UniValue result(UniValue::VOBJ);

    bool add_to_wallet{options.exists("add_to_wallet") ? options["add_to_wallet"].get_bool() : true};
    const std::string hex{EncodeHexTx(CTransaction(rawTx))};
    if (complete) {
        ...
        // Not fully signed: hand back the partially-signed transaction hex.
        result.pushKV("hex", hex);
    }
...
RPCMethod sendtoaddress()
{
    return RPCMethod{
        ...
        HELP_REQUIRING_PASSPHRASE,
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The tessera address to send to."},
                    ...
                    {"subtractfeefromamount", RPCArg::Type::BOOL, RPCArg::Default{false}, "The fee will be deducted from the amount being sent.\n"
                                         "The recipient will receive less tesseras than you enter in the amount field."},
                ...
                RPCExamples{
                    ...
                    "\nSend 0.1 FLO with a confirmation target of 6 blocks in economical fee estimate mode using positional arguments\n"
                    + HelpExampleCli("sendtoaddress", "\"" + EXAMPLE_ADDRESS[0] + "\" 0.1 \"donation\" \"sean's outpost\" false true 6 economical") +
                    "\nSend 0.5 FLO with a fee rate of 25 " + CURRENCY_ATOM + "/vB using named arguments\n"
        ...
        HELP_REQUIRING_PASSPHRASE,
                {
                    ...
                    {"amounts", RPCArg::Type::OBJ_USER_KEYS, RPCArg::Optional::NO, "The addresses and amounts",
                        {
                            {"address", RPCArg::Type::AMOUNT, RPCArg::Optional::NO, "The tessera address is the key, the numeric amount (can be string) in " + CURRENCY_UNIT + " is the value"},
                        },
                    },
                    ...
                    {"subtractfeefrom", RPCArg::Type::ARR, RPCArg::Optional::OMITTED, "The addresses.\n"
                                       ...
                                       "Those recipients will receive less tesseras than you enter in their corresponding amount field.\n"
...
static std::vector<RPCArg> FundTxDoc(bool solving_data = true)
{
    ...
    if (solving_data) {
        // Tessera has no descriptor solving data; only raw pubkeys and scripts.
...
CreatedTransactionResult FundTransaction(CWallet& wallet, const CMutableTransaction& tx, const std::vector<CRecipient>& recipients, const UniValue& options, CCoinControl& coinControl, bool override_min_fee)
{
    ...
    if (!options.isNull()) {
        if (options.type() == UniValue::VBOOL) {
            ...
            if (options.exists("changeAddress") || options.exists("change_address")) {
                ...
                CTxDestination dest = DecodeDestination(change_address_str, Params());

                if (!IsValidDestination(dest)) {
                    throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Change address must be a valid tessera address");
                }
    ...
    if (options.exists("solving_data")) {
        ...
        // Tessera has no output descriptors, so the "descriptors" solving_data
    ...
    if (options.exists("input_weights")) {
        for (const UniValue& input : options["input_weights"].get_array().getValues()) {
            ...
            if (weight < min_input_weight) {
                throw JSONRPCError(RPC_INVALID_PARAMETER, "Invalid parameter, weight cannot be less than the weight of an empty input");
            }
...
RPCMethod fundrawtransaction()
{
    return RPCMethod{
        ...
        "If the transaction has no inputs, they will be automatically selected to meet its out value.\n"
                ...
                "if the transaction has unconfirmed inputs.\n",
                {
                    ...
                    {"options", RPCArg::Type::OBJ_NAMED_PARAMS, RPCArg::Optional::OMITTED, "",
                        Cat<std::vector<RPCArg>>(
                        {
                            ...
                            {"changeAddress", RPCArg::Type::STR, RPCArg::DefaultHint{"automatic"}, "The tessera address to receive the change"},
...
//! Build the JSON result of a wallet signing attempt (Tessera has no
//! SignTransactionResultToJSON helper; Core keeps it in rawtransaction_util).
static void SignTransactionResultToJSON(CMutableTransaction& mtx, bool complete, const std::map<int, bilingual_str>& input_errors, UniValue& result)
{
    UniValue vErrors(UniValue::VARR);
    for (const auto& err_pair : input_errors) {
        if (err_pair.second.original == "Missing amount") {
            // This particular error needs to be excluded from the response.
            continue;
        }
        UniValue entry(UniValue::VOBJ);
        entry.pushKV("txid", mtx.vin.at(err_pair.first).prevout.hash.ToString());
        entry.pushKV("vout", uint64_t(mtx.vin.at(err_pair.first).prevout.n));
        UniValue witness(UniValue::VARR);
        for (const auto& stack_item : mtx.vin.at(err_pair.first).scriptWitness.stack) {
            witness.push_back(HexStr(stack_item));
        }
        entry.pushKV("witness", std::move(witness));
        entry.pushKV("scriptSig", HexStr(mtx.vin.at(err_pair.first).scriptSig));
        entry.pushKV("sequence", uint64_t(mtx.vin.at(err_pair.first).nSequence));
        entry.pushKV("error", err_pair.second.original);
        vErrors.push_back(std::move(entry));
    }
        ...
        [](const RPCMethod& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    UniValue result(UniValue::VOBJ);
    SignTransactionResultToJSON(mtx, complete, input_errors, result);
    return result;
},
    };
}
...
static std::vector<RPCArg> OutputsDoc()
{
    return
    {
        {"", RPCArg::Type::OBJ_USER_KEYS, RPCArg::Optional::OMITTED, "",
            {
                {"address", RPCArg::Type::AMOUNT, RPCArg::Optional::NO, "A key-value pair. The key (string) is the tessera address,\n"
            ...
            {"options", RPCArg::Type::OBJ_NAMED_PARAMS, RPCArg::Optional::OMITTED, "",
                {
                    ...
                    {"replaceable", RPCArg::Type::BOOL, RPCArg::Default{true},
                             "Whether the new transaction should be marked bip-125 replaceable.\n"},
                    {"estimate_mode", RPCArg::Type::STR, RPCArg::Default{"unset"}, "The fee estimate mode, must be one of (case insensitive):\n"
                              + FeeModesDetail(std::string("economical mode is used if the transaction is replaceable;\notherwise, conservative mode is used"))},
                    ...
                    {"original_change_index", RPCArg::Type::NUM, RPCArg::DefaultHint{"not set, detect change automatically"}, "The 0-based index of the change output on the original transaction. "
                                                                                                                            "Cannot be used in combination with the 'outputs' option."},
                },
        ...
        [](const RPCMethod& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    [&](){
        switch (feebumper::CreateRateBumpTransaction(*pwallet, hash, coin_control, errors, old_fee, new_fee, mtx, /*require_mine=*/true, outputs, original_change_index)) {
...
RPCMethod send()
{
    return RPCMethod{
        ...
        "Send a transaction.\n",
        {
            ...
            {"options", RPCArg::Type::OBJ_NAMED_PARAMS, RPCArg::Optional::OMITTED, "",
                Cat<std::vector<RPCArg>>(
                {
                    ...
                    {"change_address", RPCArg::Type::STR, RPCArg::DefaultHint{"automatic"}, "The tessera address to receive the change"},
        ...
        RPCExamples{""
        "\nSend 0.1 FLO with a confirmation target of 6 blocks in economical fee estimate mode\n"
        + HelpExampleCli("send", "'{\"" + EXAMPLE_ADDRESS[0] + "\": 0.1}' 6 economical\n") +
        "Send 0.3 FLO with a fee rate of 25 " + CURRENCY_ATOM + "/vB using named arguments\n"
    ...
    return RPCMethod{"sendall",
        ...
        "Unconfirmed inbound UTXOs and locked UTXOs will not be spent. Sendall will respect the avoid_reuse wallet flag.\n",
        {
            {"recipients", RPCArg::Type::ARR, RPCArg::Optional::NO, "The sendall destinations. Each address may only appear once.\n"
                ...
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "A tessera address which receives an equal share of the unspecified amount."},
                    {"", RPCArg::Type::OBJ_USER_KEYS, RPCArg::Optional::OMITTED, "",
                        {
                            {"address", RPCArg::Type::AMOUNT, RPCArg::Optional::NO, "A key-value pair. The key (string) is the tessera address, the value (float or string) is the amount in " + CURRENCY_UNIT + ""},
                        },
                    },
                },
            },
        ...
        [](const RPCMethod& self, const JSONRPCRequest& request) -> UniValue
        {
            ...
            if (tx_size.vsize == -1) {
                throw JSONRPCError(RPC_WALLET_ERROR, "Unable to determine the size of the transaction, the wallet contains unsolvable inputs");
            }
            ...
            for (CTxOut& out : rawTx.vout) {
                ...
                std::string addr{EncodeDestination(dest, Params())};
