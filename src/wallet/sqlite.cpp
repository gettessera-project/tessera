...
// Tessera: fixed wallet application_id (Core uses the network magic via the
// global Params(), which Tessera does not have).
static constexpr uint32_t TESSERA_WALLET_APP_ID = 0xF10C0DA7;
...
bool SQLiteDatabase::Verify(bilingual_str& error)
{
    ...
    uint32_t net_magic = TESSERA_WALLET_APP_ID;
    if (app_id != net_magic) {
...
void SQLiteDatabase::Open(int additional_flags)
{
    ...
    if (ret != SQLITE_OK) {
        throw std::runtime_error("SQLiteDatabase: Unable to obtain an exclusive lock on the database, is it being used by another instance of Tessera?\n");
    }
    ...
    if (!table_exists) {
        ...
        uint32_t app_id = TESSERA_WALLET_APP_ID;
