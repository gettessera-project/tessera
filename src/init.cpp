...
using http_tessera::InitHTTPServer;
using http_tessera::InterruptHTTPServer;
using http_tessera::StartHTTPServer;
using http_tessera::StopHTTPServer;
 ...
 */
static const char* TESSERA_PID_FILENAME = "tesserad.pid";
/**
...
{
    return AbsPathForConfigVal(args, args.GetPathArg("-pid", TESSERA_PID_FILENAME));
}
...
void SetupServerArgs(ArgsManager& argsman, bool can_listen_ipc)
{
    ...
    argsman.AddArg("-persistmempoolv1",
                   ...
                   ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    argsman.AddArg("-pid=<file>", strprintf("Specify pid file. Relative paths will be prefixed by a net-specific datadir location. (default: %s)", TESSERA_PID_FILENAME), ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
...
bool AppInitSanityChecks(const kernel::Context& kernel)
{
    ...
    if (!MLDSA_InitSanityCheck()) {
        return InitError(strprintf(_("ML-DSA sanity check failure. %s is shutting down."), CLIENT_NAME));
    }
...
bool AppInitMain(NodeContext& node, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    node.background_init_thread = std::thread(&util::TraceThread, "initload", [=, &chainman, &args, &node] {
        ...
        // An interrupted import may return without activating genesis. Wake the
        // init thread's genesis wait, which is otherwise only notified on
        // blockTip, and that never fires when the import was interrupted before
        // activating genesis. This wakeup lets the wait observe the shutdown
        // request. (Bitcoin Core #35652.)
        if (node.notifications) WITH_LOCK(node.notifications->m_tip_block_mutex, node.notifications->m_tip_block_cv.notify_all());
