...
/*     WARNING! If you're reading this because you're learning about crypto
       ...
       merkle root. Assuming no SHA3-256 collisions, this will detect all
...
uint256 ComputeMerkleRoot(std::vector<uint256> hashes, bool* mutated) {
    ...
    while (hashes.size() > 1) {
        ...
        // Core folds the level with the SIMD-batched double-SHA256 helper
        // SHA256D64. Tessera's consensus hash is SHA3-256, so fold each 64-byte
        // pair with the two-argument Hash() (= SHA3-256(left || right)). Done in
        // place, left to right: writing index i never clobbers an unread leaf
        // because i <= 2*i.
        const size_t pairs = hashes.size() / 2;
        for (size_t i = 0; i < pairs; ++i) {
            hashes[i] = Hash(hashes[2 * i], hashes[2 * i + 1]);
        }
        hashes.resize(pairs);
    }
