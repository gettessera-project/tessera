...
util::Result<int> SighashFromStr(const std::string& sighash)
{
    static const std::map<std::string, int> map_sighash_values = {
        // Tessera has no SIGHASH_DEFAULT (a Taproot/BIP341 value; Tessera is witness-v0 only).
    ...
    while (pc < script.end()) {
        ...
        if (0 <= opcode && opcode <= OP_PUSHDATA4) {
            if (vch.size() <= static_cast<std::vector<unsigned char>::size_type>(4)) {
                ...
                // Tessera's ML-DSA signatures are not ECDSA-DER-encoded, so the sighash-type
                // decode Core attempts here (CheckSignatureEncoding) has no Tessera analog.
                str += HexStr(vch);
            }
        } else {
...
void ScriptToUniv(const CScript& script, UniValue& out, bool include_hex, bool include_address, const SigningProvider* provider)
{
    ...
    if (include_address && ExtractDestination(script, address) && type != TxoutType::PUBKEY) {
        out.pushKV("address", EncodeDestination(address, Params()));
    }
