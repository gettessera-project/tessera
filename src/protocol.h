...
#ifndef TESSERA_PROTOCOL_H
#define TESSERA_PROTOCOL_H
...
/**
 * Service flags assumed for addresses that arrive without any: the hardcoded
 * seeds and the DNS seeds.
 *
 * v2 transport can be assumed rather than discovered. Tessera has spoken the
 * ML-KEM transport since genesis and there is no population of older nodes that
 * predates it, so a listening peer supports it. Should the assumption ever be
 * wrong for one, the connection simply falls back to v1 -- the cost of guessing
 * wrong is one reconnect, while the cost of not guessing is that every seeded
 * peer is first contacted unencrypted.
 */
